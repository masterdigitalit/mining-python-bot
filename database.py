...
def addTask(uniqueCode, name,type,state, UserId, nextSendTime):
    if(type == 'periodic'):
        cursor.execute("INSERT INTO  Tasks (uniqueCode, Name,Type,State,DateCreated,BlockTime, NextSendDate, UserId ) VALUES ( ?, ?, ?, ?,?,?,?, ?) ", [uniqueCode, name,type,state,datetime.now().strftime('%d.%m.%Y %H:%M') , (datetime.now() + timedelta(minutes=10)).strftime('%d.%m.%Y %H:%M'),  (datetime.now() + timedelta(minutes=nextSendTime)).strftime('%d.%m.%Y %H:%M'), UserId])
    else:
         cursor.execute(
...
# statesCheker
def getWarning():
    result = cursor.execute("SELECT * FROM Tasks WHERE BlockTime = ? AND State = 'not-activated'", [datetime.now().strftime('%d.%m.%Y %H:%M')])

    return result.fetchall()
def updateBlockTime(uniqueCode):
    result = cursor.execute("UPDATE  Tasks  SET BlockTime = ?  WHERE uniqueCode = ?", [     (datetime.now() + timedelta(minutes=10)).strftime('%d.%m.%Y %H:%M'),uniqueCode])


    con.commit()
def getFailed():
    result = cursor.execute("SELECT * FROM Tasks WHERE BlockTime = ? AND State = 'warning'", [datetime.now().strftime('%d.%m.%Y %H:%M')])
    return result.fetchall()
